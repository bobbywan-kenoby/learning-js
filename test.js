...
class Table {
	...
	addRow(contenu){
    	...
    	this.onmove = true;
			...
			//document.body.appendChild(this.dragElement.move());
			console.log(newRow.getRow());
    });
		newRow.getRow().addEventListener("mouseup", event => {
      console.log("clack");
			//document.body.removeChild(this.dragElement.getCopiForMove());
      ...
      if (this.onmove === false) return ;
		...
		this.myTable.addEventListener("mousemove", event => {
      if (this.onmove === false) return ;
			//this.dragElement.getCopiForMove().style.top = event.clientY-(this.dragElement.getCopiForMove().style.height/2)+"px";
	    //console.log(event.clientY-(this.dragElement.getCopiForMove().style.height/2)+"px");
    });
	}
}
...
class Row{
	...
	/*
	move(){
		this.copiForMove = document.createElement("div");
		this.copiForMove.appendChild(this.myRow.cloneNode(true));
		this.copiForMove.style.position = "absolute";
		this.copiForMove.style.width = this.myRow.style.width;
		return this.copiForMove;
	}
	getCopiForMove(){
		return this.copiForMove;
	}
